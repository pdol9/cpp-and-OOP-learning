...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2022/12/23 20:12:18 by pdolinar          #+#    #+#             */
/*   Updated: 2022/12/23 20:12:19 by pdolinar         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ClapTrap::attack(const std::string& target)
{
	if (_energyPoints < 1 || CHECK_UINT(_hitPoints)) {
		std::cout << _name << " is unable to attack >> Out of Energy / Hit points" << std::endl;
		return;
	}
...
void	ClapTrap::takeDamage(unsigned int amount)
{
	_hitPoints -= amount;
	if (_energyPoints < 1 || CHECK_UINT(_hitPoints)) {
		std::cout << _name << " is unable to respond --> Out of Energy / Hit points" << std::endl;
		return;
	}
	std::cout << _name << " took " << amount << " points of damage after being attacked" << std::endl;
...
{
	if (_energyPoints < 1 || CHECK_UINT(_hitPoints)) {
		std::cout << _name << " is unable to self-repaire by " << amount << " --> Out of Energy / Hit points" << std::endl;
		return;
	}
	std::cout << _name << " is self-repairing by " << amount << std::endl;
	_hitPoints += amount;
	_energyPoints--;
